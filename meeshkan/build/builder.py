...
from openapi_typed_2 import Info, Paths, MediaType, Header, OpenAPIObject, PathItem, Response, Operation, Parameter, \
    Reference, Server, Responses, RequestBody
...
_DEFAULT_PATH_ITEM = {'servers': None, 'parameters': None, 'get': None, 'put': None, 'post': None, 'delete': None,
                      'options': None, 'head': None, 'patch': None, 'trace': None, '_ref': None, '_x': None}
...
def build_request_body(exchange: HttpExchange, mode: UpdateMode) -> Optional[RequestBody]:
...
def update_response(response: Response, mode: UpdateMode, exchange: HttpExchange) -> Response:
    ...
    useable_headers: Mapping[str, str] = {k: v for k, v in exchange.response.headers.items() if
                                          k not in ['content-type', 'content-length', 'Content-Type',
                                                    'Content-Length'] and isinstance(v, str)}
    new_headers = {
        **response_headers,
        **{
            k: Header(
                schema=update_text_schema(v, mode, schema=response.headers.get(k, None))) for k, v in
            useable_headers.items()
        }
    }
    ...
    if response_content is not None and media_type_key in response_content:
        ...
        media_type = update_media_type(
            body=exchange.response.body, mode=mode, type_key=media_type_key, media_type=existing_media_type)
    else:
...
def update_request_body(request_body: RequestBody, mode: UpdateMode, exchange: HttpExchange) -> RequestBody:
    ...
    if request_content is not None and media_type_key in request_content:
        ...
        media_type = update_media_type(
            body=exchange.request.body, mode=mode, type_key=media_type_key, media_type=existing_media_type)
    else:
        media_type = build_media_type(
...
def update_operation(operation: Operation, exchange: HttpExchange, mode: UpdateMode) -> Operation:
    ...
    existing_parameters: Sequence[Parameter] = [x for x in operation.parameters if not isinstance(x,
                                                                                                  Reference)] if operation.parameters is not None else []
    request_query_params: Mapping[str, Union[str, Sequence[str]]] = exchange.request.query
    request_header_params: Mapping[str, Union[str, Sequence[str]]] = exchange.request.headers
    ...
    return replace(operation, responses={**operation.responses, **new_responses}, parameters=updated_parameters, requestBody=request_body)
...
def update_openapi(schema: OpenAPIObject, exchange: HttpExchange, mode: UpdateMode) -> OpenAPIObject:
    ...
    if path_match_result is not None:
        ...
        if (pathname_to_be_replaced_with_wildcard is not None):
            if mode == UpdateMode.GEN:
                ...
                schema_paths = {k: v for k, v in [(pathname_with_wildcard, pointer_to_value), *schema_paths.items()] if
                                k != pathname_to_be_replaced_with_wildcard}
                parameters_to_assign_to_pathname_with_wildcard = [] if schema_paths[
                                                                           pathname_with_wildcard].parameters is None else \
                ...
                schema_paths = {
                    **schema_paths,
                    pathname_with_wildcard: replace(schema_paths[pathname_with_wildcard],
    ...
    new_paths = {path_match_result.pathname_with_wildcard if (mode == UpdateMode.GEN) and (
                (path_match_result is not None) and (
                    path_match_result.pathname_with_wildcard is not None)) else normalized_pathname: path_item} if make_new_paths else {}
...
async def build_schema_async(async_iter: AsyncIterable[HttpExchange], mode: UpdateMode, starting_spec: OpenAPIObject) -> \
...
def build_schema_online(requests: Iterable[HttpExchange], mode: UpdateMode,
                        base_schema: OpenAPIObject = BASE_SCHEMA) -> OpenAPIObject:
...
def build_schema_batch(requests: List[HttpExchange], mode: UpdateMode = UpdateMode.GEN,
                       base_schema: OpenAPIObject = BASE_SCHEMA) -> OpenAPIObject:
