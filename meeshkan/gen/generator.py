...
def get_required_request_query_or_header_parameters_internal(
    ...
    return [
        replace(
            ...
            else Schema(**{**_SCHEMA_DEFAULT, "_type": "string"}),
        )
...
def use_if_header_last_mile(
    ...
    return (
        ...
        if r is None
            ...
            p.name,
            r if r is not None else Schema(**{**_SCHEMA_DEFAULT, "_type": "string"}),
        )
    )
...
def use_if_header(o: OpenAPIObject, p: Parameter,) -> Optional[Tuple[str, Schema]]:
    return (
        ...
        if p._in != "header"
            ...
            p,
            Schema(**{**_SCHEMA_DEFAULT, "_type": "string"})
            if p.schema is None
...
def headers_schemas_from_operation(
    schema: OpenAPIObject, operation: Operation
) -> Sequence[Schema]:
    return (
        parameters_o.Each()
        .Prism(
            lambda s: s
            if isinstance(s, Reference)
            else get_parameter_from_ref(schema, ref_name(s))
            if isinstance(s, Reference)
            else None,
            lambda a: a,
            ignore_none=True,
        )
        .Iso(lambda a: (use_if_header(schema, a), a), lambda b: b[1])[0]
        .Prism(*_prism_o, ignore_none=True)
        .collect()(operation)
    )


def make_lens_to_response_starting_from_operation(
    schema: OpenAPIObject, code: str, l: lenses.ui.BaseUiLens[S, T, X, Y] = lens
) -> lenses.ui.BaseUiLens[S, T, X, Y]:
    return l.add_lens(responses_o).Prism(
        lambda s: s
        if isinstance(s, Response)
        else get_response_from_ref(schema, ref_name(s))
        if isinstance(s, Reference)
        else None,
        lambda a: a,
        ignore_none=True,
...
def header_schemas_from_response(
    schema: OpenAPIObject, operation: Operation, code: str
) -> Sequence[Schema]:
    return (
        ...
        .Items()
        .Prism(
            lambda a: string_header(a[0], get_header_from_ref(schema, ref_name(a[1])))
            if isinstance(a[1], Reference)
            else (a[0], a[1]),
            lambda a: a,
            ignore_none=True,
        )
        .Iso(lambda a: replace(a[1], name=a[0], _in="header"), lambda a: (a.name, a))
        .add_lens(parameter_schema(schema))
        .collect()(operation)
    )
...
def fake_object(schema: Any, top_schema: Any, depth: int) -> Any:
        ...
        if "additionalProperties" not in schema
            ...
            for k, v in [
                (
                    ...
                    else faker(schema["additionalProperties"], top_schema, depth),
                )
    ...
    return {
        ...
        **{
            k: v
            for k, v in [
                (p, faker(schema["properties"][p], top_schema, depth))
                for p in properties
            ]
        },
    }
...
def fake_array(schema: Any, top_schema: Any, depth: int) -> Any:
    ...
    return (
        ...
        else [faker(x, top_schema, depth) for x in schema["items"]]
        if type(schema["items"]) is type([])
        else [
            faker(schema["items"], top_schema, depth)
...
def fake_all_of(schema: Any, top_schema: Any, depth: int) -> Any:
    return reduce(
        lambda a, b: {**a, **b},
        [faker(x, top_schema, depth) for x in schema["allOf"]],
        {},
    )
