...
def _request_from_tshark(obj) -> Request:
    ...
    return RequestBuilder.from_dict(
        dict(
            body=body,
            protocol=protocol,
            method=method,
...
def _response_from_tshark(obj) -> Response:
    ...
    return ResponseBuilder.from_dict(
        dict(bodyAsJson=None, statusCode=status_code, body=body, headers=headers)
    )
...
def _parse_headers(header_line: str) -> Dict[str, Union[str, List[str]]]:
    ...
    return dict(
        tupl for line in lines for tupl in (line_to_tuple(line),) if tupl is not None
    )
...
def convert_pcap(filepath: str) -> Iterator[HttpExchange]:
    ...
    if filepath == "-":
        raise NotImplementedError(
            "File path '-' detected. Reading from stdin is not implemented yet."
        )
    ...
    tshark_proc = subprocess.run(tshark_cmd, shell=True, stdout=subprocess.PIPE)

    if tshark_proc.returncode != 0:
        raise Exception("tshark exited with code {}".format(tshark_proc.returncode))
