...
@click.option(
    ...
    "--header_routing",
    is_flag=True,
    help="Whether to use a header based routing to a target host.",
)
@click.option(
    "-s", "--specs_dir", default="./specs", help="Directory to store OpenAPI specs."
)
@click.option(
    "-m",
    "--mode",
...
def record(port, admin_port, log_dir, header_routing, specs_dir, mode):
    ...
    with RequestLoggingCallback(
        log_dir=log_dir,
        specs_dir=specs_dir,
        update_mode=UpdateMode[mode.upper()] if mode else None,
    ) as callback:
        server = RecordProxy(
            callback, HeaderRouting() if header_routing else PathRouting()
        )
        server.listen(port)
...
@click.option("-a", "--admin_port", default="8888", help="Admin server port.")
@click.option(
    "-s", "--specs_dir", default="./specs", help="Directory with OpenAPI schemas."
)
@click.option(
    "-r",
    "--header_routing",
...
def mock(callback_path, admin_port, port, specs_dir, header_routing):
    ...
    app = make_mocking_app(
        callback_path, specs_dir, HeaderRouting() if header_routing else PathRouting()
    )
    http_server = HTTPServer(app)
    http_server.listen(port)
