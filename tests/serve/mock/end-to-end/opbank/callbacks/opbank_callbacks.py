...
def confirm(request_body, response_body, storage):
    ...
    storage[payment_info["receiverIban"]] = (
        storage.get(payment_info["receiverIban"], 0) + payment_info["amount"]
    )
    storage[payment_info["payerIban"]] = (
        storage.get(payment_info["payerIban"], 0) - payment_info["amount"]
    )
    return response_body
