...
from meeshkan.schemabuilder import (
    build_schema_batch,
    update_openapi,
    build_schema_online,
)
...
from ..util import (
    petstore_schema,
    read_recordings_as_request_response,
    POKEAPI_RECORDINGS_PATH,
)
...
def test_items(schema):
    ...
    assert_that(
        properties,
        has_entry(
            "clone_url", equal_to(Schema(**{**_SCHEMA_DEFAULT, "_type": "string"}))
        ),
    )
...
def test_schema_in_replay_mode():
    reqs = []
    with open(
        "tests/server/mock/callbacks/opbank/recordings/recording.jsonl", "r"
    ) as rr:
        reqs = rr.read().split("\n")
