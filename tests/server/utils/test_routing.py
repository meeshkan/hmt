from meeshkan.server.utils.routing import PathRouting, HeaderRouting
...
def test_path_routing():
    ...
    res = routing.route('/http://api.com/api/v1/entity/update', {'Host': 'localhost:8000'})
    assert 'api.com' == res.hostname
    assert 'api.com' == res.host
    assert 80 == res.port
    assert '/api/v1/entity/update' == res.path
...
def test_header_routing():
    ...
    res = routing.route('/api/v1/entity/update', {'Host': 'api.com'}, inbound_scheme='https')
    assert 'api.com' == res.hostname
    assert 'api.com' == res.host
    assert 443 == res.port
    assert '/api/v1/entity/update' == res.path
    ...
    res = routing.route('/api/v1/entity/update', {'Host': 'localhost:8443'}, inbound_scheme='https')
    assert 'localhost' == res.hostname
    assert 'localhost:8443' == res.host
    assert 8443 == res.port
    assert '/api/v1/entity/update' == res.path
